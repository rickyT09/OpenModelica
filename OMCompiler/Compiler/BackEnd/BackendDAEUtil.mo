...
algorithm
  (outExp, cont, outTpl) := matchcontinue (inExp, inTpl)
    ...
    case (DAE.CREF(componentRef=cr), (vars, globalKnownVars, _)) algorithm
...
protected function markStateEquationsWork
  ...
  Does the job by looking at variable indexes and adjacency matrix.
  inputs: (eqns: int list,
             marks: (int array  BackendDAE.AdjacencyMatrix  int vector  int vector))
  outputs: ((marks: int array  BackendDAE.AdjacencyMatrix
...
"author: PA
  ...
  inputs:  (AdjacencyMatrixT, int /* variable */)
  outputs:  int list /* equations */"
  input BackendDAE.AdjacencyMatrixT inAdjacencyMatrixT;
  input Integer inInteger;
  output list<Integer> outIntegerLst;
algorithm
  outIntegerLst := matchcontinue (inAdjacencyMatrixT,inInteger)
    local
      Integer n,indx;
...
"author: PA
  ...
  inputs:  (AdjacencyMatrix, int /* equation */)
...
algorithm
  ...
  for eqSystem in inBackendDAE.eqs loop
    ...
    try
      ...
      (eqSystem, adjMatrix, adjMatrixT) := getAdjacencyMatrixfromOption(eqSystem, BackendDAE.ABSOLUTE(), SOME(inBackendDAE.shared.functionTree), BackendDAEUtil.isInitializationDAE(inBackendDAE.shared));
  ...
  oSyst := match iSyst
    case syst as BackendDAE.EQSYSTEM( orderedEqs=ordererdEqs, orderedVars=vars,
      ...
      algorithm
        ...
        (_, m, _) := getAdjacencyMatrix(iSyst, BackendDAE.SPARSE(), SOME(funcs), isInitializationDAE(shared));
  ...
  for system in systems loop
    ...
    for v in BackendVariable.varList(vars) loop
      if not BackendVariable.isVarOnTopLevelAndOutput(v) then
         ...
         */
        if BackendVariable.isRealVar(v) then
...
algorithm
  try
    ...
    (outAdjacencyMatrix, outAdjacencyMatrixT) :=
      adjacencyMatrixDispatch(vars, eqns, inIndexType, functionTree, isInitial);
  else
    Error.addMessage(Error.INTERNAL_ERROR, {"BackendDAEUtil.adjacencyMatrix failed."});
    fail();
  end try;
end adjacencyMatrix;

public function adjacencyMatrixMasked
  input BackendDAE.EqSystem inEqSystem;
  input BackendDAE.IndexType inIndexType;
...
algorithm
  try
    BackendDAE.EQSYSTEM(orderedVars = vars, orderedEqs = eqns) := inEqSystem;
    (outAdjacencyMatrix, outAdjacencyMatrixT) :=
      adjacencyMatrixDispatchMasked(vars, eqns, inIndexType, inMask, functionTree, isInitial);
  else
    Error.addMessage(Error.INTERNAL_ERROR, {"BackendDAEUtil.adjacencyMatrix failed."});
    fail();
  end try;
end adjacencyMatrixMasked;

public function adjacencyMatrixScalar
"author: PA, adrpo
  Calculates the adjacency matrix, i.e. which variables are present in each equation.
  You can ask for absolute indexes or normal (negative for der) via the IndexType"
  input BackendDAE.EqSystem syst;
...
algorithm
  try
    ...
    (outAdjacencyMatrix, outAdjacencyMatrixT, outMapEqnIncRow, outMapIncRowEqn) :=
      adjacencyMatrixDispatchScalar(vars, eqns, inIndexType, functionTree, isInitial);
  else
    Error.addMessage(Error.INTERNAL_ERROR, {"BackendDAEUtil.adjacencyMatrixScalar failed."});
    fail();
  end try;
...
public function adjacencyMatrixDispatch
"@author: adrpo
...
algorithm
  ...
  outAdjacencyArray := arrayCreate(num_eqs, {});
  outAdjacencyArrayT := arrayCreate(num_vars, {});
...
end adjacencyMatrixDispatch;

public function adjacencyMatrixDispatchMasked
  input BackendDAE.Variables inVars;
  input BackendDAE.EquationArray inEqns;
  input BackendDAE.IndexType inIndexType;
  input array<Boolean> inMask;
...
end adjacencyMatrixDispatchMasked;

protected function adjacencyMatrixDispatchScalar
"@author: adrpo
  Calculates the adjacency matrix as an array of list of integers"
  input BackendDAE.Variables inVars;
  input BackendDAE.EquationArray inEqns;
...
algorithm
  ...
  for idx in 1:num_eqs loop
    ...
    outAdjacencyArrayT := filladjacencyMatrixT(row, row_indices, outAdjacencyArrayT);
  end for;

  outAdjacencyArray := List.listArrayReverse(iarr);
  omapIncRowEqn := List.listArrayReverse(imap);
end adjacencyMatrixDispatchScalar;

protected function filladjacencyMatrixT
"@author: Frenkel TUD 2011-04
  inserts the equation numbers"
  input BackendDAE.AdjacencyMatrixElement eqns;
  input list<Integer> eqnsindxs;
  input BackendDAE.AdjacencyMatrixT inAdjacencyArrayT;
...
algorithm
  ...
  for v in eqns loop
    ...
    // Put it in the array.
    row := listAppend(ei, arrayGet(inAdjacencyArrayT, v));
    arrayUpdate(outAdjacencyArrayT, v, row);
  end for;
end filladjacencyMatrixT;

public function adjacencyRow
"author: PA
  Helper function to adjacencyMatrix. Calculates the indidence row
  ...
  */
  ...
  (outIntegerLst,rowSize) := matchcontinue (inlinedEquation)
    ...
    case BackendDAE.EQUATION(exp = e1,scalar = e2)
      algorithm
        lst1 := adjacencyRowExp(e1, vars, iRow, functionTree, inIndexType, isInitial);
        res := adjacencyRowExp(e2, vars, lst1, functionTree, inIndexType, isInitial);
      then
        (res,1);
    ...
    case BackendDAE.FOR_EQUATION(body = eqn, iter = DAE.CREF(componentRef = DAE.CREF_IDENT(ident = str)))
      ...
      then
        adjacencyRow(eqn, vars, inIndexType, functionTree, iRow, isInitial);

    // SOLVED_EQUATION
    case BackendDAE.SOLVED_EQUATION(componentRef = cr,exp = e)
      algorithm
        expCref := Expression.crefExp(cr);
        lst1 := adjacencyRowExp(expCref, vars, iRow, functionTree, inIndexType, isInitial);
        res := adjacencyRowExp(e, vars, lst1, functionTree, inIndexType, isInitial);
      then
        (res,1);
    ...
    case BackendDAE.RESIDUAL_EQUATION(exp = e)
      algorithm
        res := adjacencyRowExp(e, vars, iRow, functionTree, inIndexType, isInitial);
      then
        (res,1);
    ...
    case BackendDAE.WHEN_EQUATION(size=size,whenEquation = we)
      algorithm
        res := adjacencyRowWhen(we, vars, inIndexType, functionTree, iRow, isInitial);
      then
        (res,size);
    ...
    */
    case BackendDAE.IF_EQUATION(conditions={DAE.CALL(path=Absyn.IDENT("initial"))},eqnstrue=eqnslst,eqnsfalse=eqns)
      algorithm
        ...
        (res,size) := adjacencyRowLst(eqns, vars, inIndexType, functionTree, iRow, isInitial);
      then
        (res,size);
    ...
    */
    case BackendDAE.IF_EQUATION(conditions=expl,eqnstrue=eqnslst,eqnsfalse=eqns)
      algorithm
        res := adjacencyRow1(expl, adjacencyRowExp, vars, iRow, functionTree, inIndexType, isInitial);
        res := adjacencyRowLstLst(eqnslst, vars, inIndexType, functionTree, res, isInitial);
        (res,size) := adjacencyRowLst(eqns, vars, inIndexType, functionTree, res, isInitial);
      then
        (res,size);
...
protected function adjacencyRowLst
"author: Frenkel TUD
  Helper function to adjacencyMatrix. Calculates the indidence row
...
algorithm
  for eq in inEquation loop
    (outIntegerLst, size) :=
      adjacencyRow(eq, inVariables, inIndexType, functionTree, outIntegerLst, isInitial);
    rowSize := rowSize + size;
  end for;
end adjacencyRowLst;

protected function adjacencyRowLstLst
"author: Frenkel TUD
  Helper function to adjacencyMatrix. Calculates the indidence row
  in the matrix for if equation."
...
algorithm
  for eql in inEquation loop
    (outIntegerLst, size) := adjacencyRowLst(eql, inVariables, inIndexType,
        functionTree, outIntegerLst, isInitial);
    rowSize := rowSize + size;
  end for;
end adjacencyRowLstLst;

protected function adjacencyRowWhen
"Helper function to adjacencyMatrix. Calculates the indidence row
...
algorithm
  outRow := match (inEquation)
    ...
    case BackendDAE.WHEN_STMTS(condition = cond, whenStmtLst = whenStmtLst, elsewhenPart = oelsewe)
      algorithm
        outRow := adjacencyRowExp(cond, inVariables, inRow, functionTree, inIndexType, isInitial);
        outRow := adjacencyRowWhenOps(whenStmtLst, inVariables, inIndexType, functionTree, outRow, isInitial);
...
end adjacencyRowWhen;

protected function adjacencyRowWhenOps
"Helper function to adjacencyMatrix. Calculates the indidence row
...
algorithm
  outRow := match (inWhenOps)
    ...
    case (BackendDAE.ASSIGN(left = DAE.CREF(componentRef = DAE.WILD()), right = e2)::rest)
      algorithm
        outRow := adjacencyRowExp(e2, inVariables, inRow, functionTree, inIndexType, isInitial);
        outRow := adjacencyRowWhenOps(rest, inVariables, inIndexType, functionTree, outRow, isInitial);
    then outRow;

    case (BackendDAE.ASSIGN(left = e1, right = e2)::rest)
      algorithm
        //e1 = Expression.crefExp(cr);
        outRow := adjacencyRowExp(e1, inVariables, inRow, functionTree, inIndexType, isInitial);
        outRow := adjacencyRowExp(e2, inVariables, outRow, functionTree, inIndexType, isInitial);
        outRow := adjacencyRowWhenOps(rest, inVariables, inIndexType, functionTree, outRow, isInitial);
    then outRow;
...
end adjacencyRowAlgorithm;

public function adjacencyRow1
  ...
  outArg1 := match(inList)
    ...
    case e1::rest_e1
      algorithm
        ...
        res1 := adjacencyRow1(rest_e1, inFunc, inArg, res, inArg2, inArg3, inArg4);
      then
        res1;
  end match;
end adjacencyRow1;

public function adjacencyRowExp "author: PA
  Helper function to adjacencyRow, investigates expressions for variables,
...
algorithm
  outIntegerLst := match inIndexType
    ...
    case BackendDAE.BASECLOCK_IDX() algorithm
      (_, (_, vallst, _)) := Expression.traverseExpTopDown(inExp, traversingAdjacencyRowExpFinderBaseClock, (inVariables, inIntegerLst, isInitial));
    then vallst;

    case BackendDAE.SUBCLOCK_IDX() algorithm
...
protected function traversingadjacencyRowIfExpSolvableFinder
  ...
  Checks if condition for special treatment, helper for only traversingadjacencyRowExpSolvableFinder.
    ...
    case DAE.IFEXP(expCond = expCond, expThen = expThen, expElse = expElse)
      ...
      algorithm
        if Flags.isSet(Flags.FAILTRACE) then
          Error.addMessage(Error.GENERIC_ELAB_EXPRESSION, {"[BackendDAEUtil.traversingadjacencyRowIfExpSolvableFinder]: " + ExpressionDump.dumpExpStr(e, 0) + ": If-Expression could not be evaluated."});
        end if;
      then fail();
  end matchcontinue;
end traversingadjacencyRowIfExpSolvableFinder;

protected function traversingadjacencyRowIfExp
  "author: kabdelhak FHB 2020-01
  Checks if condition for special treatment, helper for all traversingadjacencyRowExp
  functions except traversingadjacencyRowExpSolvableFinder.
      special treatment for inital() condition.
      Ticket: #5795
      ToDo: inside more complex expression? IF_EQUATION?"
  input output DAE.Exp e;
...
algorithm
  tpl := matchcontinue e
    ...
    case DAE.IFEXP(expCond = expCond, expThen = expThen, expElse = expElse)
      ...
      algorithm
        if Flags.isSet(Flags.FAILTRACE) then
          Error.addMessage(Error.GENERIC_ELAB_EXPRESSION, {"[BackendDAEUtil.traversingadjacencyRowIfExp]: " + ExpressionDump.dumpExpStr(e, 0) + ": If-Expression could not be evaluated."});
        end if;
      then fail();
  end matchcontinue;
...
protected function traversingAdjacencyRowIfExpEnhanced
  "author: kabdelhak FHB 2020-01
  Checks if condition for special treatment, helper for traversingAdjacencyRowExpSolvableEnhancedFinder.
      special treatment for initial() condition.
      Ticket: #5795
      ToDo: inside more complex expression?"
  input output DAE.Exp e;
  output Boolean cont = false; // always false, just for convenience
  input output tuple<BackendDAE.Variables, Boolean, Boolean, Integer, array<Integer>, list<Integer>> tpl;
  input traverserFunction traFunc;
  partial function traverserFunction
    input output DAE.Exp exp;
    output Boolean cont;
...
algorithm
  tpl := matchcontinue e
    local
      DAE.Exp expCond, expThen, expElse;
      Boolean isInitial, conditionTrue, bs;
      BackendDAE.Variables vars;
      Integer it, mark;
      array<Integer> at, rowmark;
      list<Integer> pa;
      BinaryTree.BinTree bt;
    case DAE.IFEXP(expCond = expCond, expThen = expThen, expElse = expElse) guard(Expression.containsInitialCall(expCond))
      algorithm
        (_, _, isInitial, _, _, _) := tpl;
        conditionTrue := match expCond
          case DAE.CALL(path=Absyn.IDENT("initial")) then isInitial;
          case DAE.LUNARY(operator = DAE.NOT(), exp = DAE.CALL(path=Absyn.IDENT("initial"))) then not isInitial;
          else fail(); // Lack of information, do not consider it as an initial call and traverse all branches
    ...
    case DAE.IFEXP(expCond = expCond, expThen = expThen, expElse = expElse)
      ...
      algorithm
        if Flags.isSet(Flags.FAILTRACE) then
          Error.addMessage(Error.GENERIC_ELAB_EXPRESSION, {"[BackendDAEUtil.traversingAdjacencyRowIfExpEnhanced]: " + ExpressionDump.dumpExpStr(e, 0) + ": If-Expression could not be evaluated."});
        end if;
      then fail();
  end matchcontinue;
end traversingAdjacencyRowIfExpEnhanced;
...
end traversingAdjacencyRowExpFinderBaseClock;

public function traversingAdjacencyRowExpFinderSubClock "author: lochel
...
end traversingAdjacencyRowExpFinderSubClock;
...
algorithm
  ...
  local
    ...
    case (DAE.CALL(path=Absyn.IDENT(name="homotopy"), expLst = {e1, e2}), (_, _, true))
      algorithm
        (_, b1, tpl) := traversingadjacencyRowExpFinderwithInput(e1, inTpl);
        (_, b2, tpl) := traversingadjacencyRowExpFinderwithInput(e2, tpl);
    then (inExp, b1 and b2, tpl);
...
end traversingadjacencyRowExpFinderwithInput;
...
end adjacencyRowExp1withInput;

public function updateAdjacencyMatrix
"author: PA
  Takes a daelow and the adjacency matrix and its transposed
  represenation and a list of  equation indexes that needs to be updated.
  First the BackendDAE.AdjacencyMatrix is updated, i.e. the mapping from equations
  ...
  update the BackendDAE.AdjacencyMatrixT (transpose) mapping from variables to
  equations. The function returns an updated adjacency matrix.
  inputs:  (BackendDAE,
            AdjacencyMatrix,
            AdjacencyMatrixT,
  ...
  osyst := matchcontinue syst
    ...
    case BackendDAE.EQSYSTEM(orderedVars=vars, orderedEqs=daeeqns, m=SOME(m), mT=SOME(mt), mapping=SOME(mapping))
      algorithm
        ...
        (m,mt) := updateAdjacencyMatrix1(vars, daeeqns, inIndxType, functionTree, m, mt, inIntegerLst, isInitial);
...
end updateAdjacencyMatrixScalar;

protected function updateAdjacencyMatrixScalar1
  "Helper"
  input BackendDAE.Variables vars;
  input BackendDAE.EquationArray daeeqns;
  input BackendDAE.AdjacencyMatrix m;
...
end updateAdjacencyMatrixScalar1;

protected function updateAdjacencyMatrixScalar2
  "Helper"
  input Integer index;
  input Integer n;
  input Integer size;
...
algorithm
  outAdjacencyMatrixT:=
  matchcontinue (inValue,inIntegerLst,inAdjacencyMatrixT)
    local
      BackendDAE.AdjacencyMatrixT mt,mt_1,mt_2;
      BackendDAE.AdjacencyMatrixElement mlst,mlst1;
      list<Integer> keys;
      Integer k,kabs;
      Integer v,v_1;
    case (_,{},mt) then mt;
...
algorithm
  ...
  matchcontinue (inValue,inIntegerLst,inAdjacencyMatrixT)
    local
      BackendDAE.AdjacencyMatrixT mt,mt_1,mt_2;
      BackendDAE.AdjacencyMatrixElement mlst;
      list<Integer> keys;
      Integer k,kabs;
      Integer v,v_1;
    case (_,{},mt) then mt;
...
public function getAdjacencyMatrixfromOptionForMapEqSystem "function getAdjacencyMatrixfromOption"
  input BackendDAE.EqSystem syst;
  input BackendDAE.IndexType inIndxType;
  input BackendDAE.Shared shared;
  output BackendDAE.EqSystem osyst;
...
algorithm
  ...
  oshared := shared;
end getAdjacencyMatrixfromOptionForMapEqSystem;
...
algorithm
  (outSyst, outM, outMT) := match inSyst
    local
      ...
      Boolean scalar;
      BackendDAE.AdjacencyMatrixMapping mapping;
    case BackendDAE.EQSYSTEM(m=NONE()) algorithm
      (m, mT) := adjacencyMatrix(inSyst, inIndxType, inFunctionTree, isInitial);
    ...
    case BackendDAE.EQSYSTEM(orderedVars=v, m=SOME(m), mT=NONE(), mapping=NONE()) algorithm
    ...
    case BackendDAE.EQSYSTEM(orderedVars=v, m=SOME(m), mT=NONE(), mapping=SOME(mapping)) algorithm
      mT := AdjacencyMatrix.transposeAdjacencyMatrix(m, BackendVariable.varsSize(v));
    then (BackendDAEUtil.setEqSystMatrices(inSyst, SOME(m), SOME(mT), SOME(mapping)), m, mT);
...
public function getAdjacencyMatrix "this function returns the adjacency matrix,
...
protected
  BackendDAE.AdjacencyMatrixMapping mapping;
algorithm
  (outM, outMT) := adjacencyMatrix(inEqSystem, inIndxType, functionTree, isInitial);
...
public function getAdjacencyMatrixScalar "function getAdjacencyMatrixScalar"
  ...
  output array<Integer> outMapIncRowEqn;
algorithm
  (outM, outMT, outMapEqnIncRow, outMapIncRowEqn) := adjacencyMatrixScalar(syst, inIndxType, functionTree, isInitial);
...
public function removedAdjacencyMatrix
  ...
  output BackendDAE.AdjacencyMatrix outMT;
algorithm
  (outM, outMT) := adjacencyMatrixDispatch(inSyst.orderedVars, inSyst.removedEqs, inIndxType, inFunctionTree, isInitial);
end removedAdjacencyMatrix;

public function removedAdjacencyMatrixMasked
  input BackendDAE.EqSystem inSyst;
  input BackendDAE.IndexType inIndxType;
  input array<Boolean> inMask;
  input Option<DAE.FunctionTree> inFunctionTree;
...
public function adjacencyMatrixToSparsePattern
" Applies absolute value to all entries in the adjacency matrix and
...
"author: Frenkel TUD 2012-05
  ...
  output BackendDAE.AdjacencyMatrixEnhanced outAdjacencyMatrix;
  output BackendDAE.AdjacencyMatrixTEnhanced outAdjacencyMatrixT;
  output array<list<Integer>> outMapEqnIncRow;
  output array<Integer> outMapIncRowEqn;
protected
...
algorithm
  ...
  matchcontinue (syst, shared)
    ...
    case (BackendDAE.EQSYSTEM(orderedVars = vars,orderedEqs = eqns), BackendDAE.SHARED(globalKnownVars=globalKnownVars))
      algorithm
        ...
        (arr,arrT,mapEqnIncRow,mapIncRowEqn,varsSolvedInWhenEqnsTupleList) := adjacencyMatrixDispatchEnhancedScalar(vars, eqns,arrT, numberOfEqs, rowmark,globalKnownVars ,trytosolve, shared);
      then
  ...
  makeWhenEqnVarsUnsolvable(outAdjacencyMatrix,outAdjacencyMatrixT,varsSolvedInWhenEqnsTupleList);
...
"@author: Frenkel TUD 2012-05
  ...
  output BackendDAE.AdjacencyMatrixEnhanced outAdjacencyArray;
  input output BackendDAE.AdjacencyMatrixTEnhanced adjacencyArrayT;
...
protected
  list<BackendDAE.AdjacencyMatrixElementEnhanced> inAdjacencyArray = {};
...
algorithm
  for i1 in 1:numberOfEqs loop
    ...
    // compute the row
    (row,size,varsSolvedInWhenEqnsTuple) := adjacencyRowEnhanced(vars, e, i1, rowmark, globalKnownVars, trytosolve, shared);
    rowindxs := List.intRange2(rowSize+1, rowSize+size);
    rowSize := rowSize + size;
    imapIncRowEqn := List.consN(size,i1,imapIncRowEqn);
    // put it in the arrays
    inAdjacencyArray := List.consN(size,row,inAdjacencyArray);
    adjacencyArrayT := fillincAdjacencyMatrixTEnhanced(row,rowindxs,adjacencyArrayT);
...
"author: Frenkel TUD 2012-05
  ...
  output BackendDAE.AdjacencyMatrixTEnhanced outAdjacencyMatrixT;
algorithm
  (outAdjacencyMatrix,outAdjacencyMatrixT) := matchcontinue (syst, shared)
    local
      BackendDAE.AdjacencyMatrixEnhanced arr;
      BackendDAE.AdjacencyMatrixTEnhanced arrT;
      BackendDAE.Variables vars,globalKnownVars;
    ...
    case (BackendDAE.EQSYSTEM(orderedVars = vars,orderedEqs = eqns), BackendDAE.SHARED(globalKnownVars=globalKnownVars))
      algorithm
        ...
        // create the array to mark if a variable is allready found in the equation
        rowmark := arrayCreate(numberofVars, 0);
        (arr,arrT) := adjacencyMatrixDispatchEnhanced(vars, eqns, arr,arrT, 0, numberOfEqs, intLt(0, numberOfEqs),rowmark,globalKnownVars,trytosolve,shared);
      then
        (arr,arrT);
...
"@author: Frenkel TUD 2012-05
  ...
  input BackendDAE.Variables vars;
  input BackendDAE.EquationArray eqArr;
  input BackendDAE.AdjacencyMatrixEnhanced inAdjacencyArray;
  input BackendDAE.AdjacencyMatrixTEnhanced inAdjacencyArrayT;
  input Integer index;
  input Integer numberOfEqs;
  input Boolean stop;
  input array<Integer> rowmark;
  ...
  input BackendDAE.Shared shared;
  output BackendDAE.AdjacencyMatrixEnhanced outAdjacencyArray;
  output BackendDAE.AdjacencyMatrixTEnhanced outAdjacencyArrayT;
algorithm
  (outAdjacencyArray,outAdjacencyArrayT) :=
    match (vars, eqArr, inAdjacencyArray, inAdjacencyArrayT, index, numberOfEqs, stop, rowmark, globalKnownVars)
    local
      BackendDAE.AdjacencyMatrixElementEnhanced row;
      BackendDAE.Equation e;
    ...
    then (inAdjacencyArray, inAdjacencyArrayT);
    ...
    case (_, _, _, _, _, _, true, _, _)
      algorithm
        ...
        e := BackendEquation.get(eqArr, i1);
        // compute the row
        (row,_,_) := adjacencyRowEnhanced(vars, e, i1, rowmark, globalKnownVars, trytosolve,shared);
        // put it in the arrays
        iArr := arrayUpdate(inAdjacencyArray, i1, row);
        iArrT := fillincAdjacencyMatrixTEnhanced(row,{i1},inAdjacencyArrayT);
        (iArr,iArrT) := adjacencyMatrixDispatchEnhanced(vars, eqArr, iArr, iArrT, i1, numberOfEqs, intLt(i1, numberOfEqs), rowmark, globalKnownVars, trytosolve, shared);
      then
        (iArr,iArrT);
  end match;
...
"@author: Frenkel TUD 2011-04
  ...
  output BackendDAE.AdjacencyMatrixTEnhanced outAdjacencyArrayT;
algorithm
  outAdjacencyArrayT := matchcontinue (eqns, eqnsindxs, inAdjacencyArrayT)
    local
...
protected
  Boolean isInitial;
algorithm
  isInitial := isInitializationDAE(shared);
  (outRow,size) := matchcontinue (inVariables,inEquation,mark,rowmark,globalKnownVars)
    ...
    case (vars,BackendDAE.EQUATION(exp = e1,scalar = e2),_,_,_)
      algorithm
        lst := adjacencyRowExpEnhanced(e1, vars, mark, rowmark, isInitial, {});
        lst := adjacencyRowExpEnhanced(e2, vars, mark, rowmark, isInitial, lst);
        row := adjacencyRowEnhanced1(lst,e1,e2,vars,globalKnownVars,mark,rowmark,{},trytosolve,1,shared);
      then
        (row,1);
    // COMPLEX_EQUATION
    case (vars,BackendDAE.COMPLEX_EQUATION(size=size,left=e1,right=e2),_,_,_)
      algorithm
        lst := adjacencyRowExpEnhanced(e1, vars, mark, rowmark, isInitial, {});
        lst := adjacencyRowExpEnhanced(e2, vars, mark, rowmark, isInitial, lst);
        row := adjacencyRowEnhanced1(lst,e1,e2,vars,globalKnownVars,mark,rowmark,{},trytosolve,size,shared);
      then
        (row,size);
    // ARRAY_EQUATION
    ...
    case (vars,BackendDAE.SOLVED_EQUATION(componentRef = cr,exp = e),_,_,_)
      algorithm
        expCref := Expression.crefExp(cr);
        lst := adjacencyRowExpEnhanced(expCref, vars, mark, rowmark, isInitial, {});
        lst := adjacencyRowExpEnhanced(e, vars, mark, rowmark, isInitial, lst);
        row := adjacencyRowEnhanced1(lst,expCref,e,vars,globalKnownVars,mark,rowmark,{},trytosolve,1,shared);
      then
        (row,1);
    // RESIDUAL_EQUATION
    case (vars,BackendDAE.RESIDUAL_EQUATION(exp = e),_,_,_)
      algorithm
        lst := adjacencyRowExpEnhanced(e, vars, mark, rowmark, isInitial, {});
        row := adjacencyRowEnhanced1(lst,e,DAE.RCONST(0.0),vars,globalKnownVars,mark,rowmark,{},trytosolve,1,shared);
      then
        (row,1);
    ...
    case (vars,BackendDAE.WHEN_EQUATION(size=size,whenEquation = elsewe),_,_,_)
      algorithm
        (row,varsSolvedInWhenEqns) := adjacencyRowWhenEnhanced(elsewe, mark, rowmark, vars, globalKnownVars, {}, {},shared);
    ...
    /*
      ...
      only then branch needs to be checked for initialization and else branch otherwise
    */
    case(_,BackendDAE.IF_EQUATION(conditions={DAE.CALL(path=Absyn.IDENT("initial"))},eqnstrue={eqns}, eqnsfalse=eqnselse),_,_,_)
      algorithm
        if isInitializationDAE(shared) then
          (row,size) := adjacencyRowEnhancedEqnLst(eqns,inVariables,mark,rowmark,globalKnownVars,trytosolve,shared);
        else
          (row,size) := adjacencyRowEnhancedEqnLst(eqnselse,inVariables,mark,rowmark,globalKnownVars,trytosolve,shared);
        end if;
      then
        (row,size);
...
algorithm
  ...
  for eqn in iEqns loop
    (row,size,_) := adjacencyRowEnhanced(inVariables, eqn, mark, rowmark, globalKnownVars, trytosolve,shared);
    lst := List.map(row,Util.tuple31);
...
algorithm
  ...
  lst := adjacencyRowExpEnhanced(condition, vars, mark, rowmark, isInitial, iLst);
  for rs in whenStmtLst loop
    _ := match(rs)
      ...
      case BackendDAE.ASSIGN(left = leftexp as DAE.CREF(componentRef = left), right=right)
        ...
        lst := adjacencyRowExpEnhanced(right, vars, mark, rowmark, isInitial, lst);
        // mark all negative because the when condition cannot used to solve a variable
        _ := List.fold1(lst,markNegativ,rowmark,mark);
        //leftexp = Expression.crefExp(left);
        lst := adjacencyRowExpEnhanced(leftexp, vars, mark, rowmark, isInitial, lst);
        outRow := adjacencyRowEnhanced1(lst,leftexp,right,vars,globalKnownVars,mark,rowmark,outRow,false,1,shared);
      then ();

      case BackendDAE.ASSIGN(leftexp, right) algorithm
  ...
  if isSome(oelsepart) then
    ...
    outRow := adjacencyRowWhenEnhanced(elsepart, mark, rowmark, vars, globalKnownVars, lst, outRow, shared);
  end if;
...
*/
    ...
    case(r::rest,_,_,_,_,_,_,_)
      ...
      algorithm
        ...
        (de,solved,derived,cons) := tryToSolveOrDerive(e_derAlias, cr1, vars, SOME(shared.functionTree),trytosolve);
        if not solved then
...
public function isSolvable
  input BackendDAE.Solvability solvability;
...
algorithm
  solvable := match solvability
    ...
    case BackendDAE.SOLVABILITY_LINEAR(b=b) then (b and not strict);
    else true;
  end match;
end isSolvable;
  ...
  oSolvab := match(b1,b2,cr,e,crlst,vars,globalKnownVars)
    ...
    case(true,_,_,_,_,_,_)
      algorithm
        ...
        b_1 := Expression.isConst(e1) or BackendVariable.isKnownAndParam(e1,globalKnownVars);
      then
...
algorithm
  (_,(_,_,_,_,_,outRow)) := Expression.traverseExpTopDown(inExp, traversingAdjacencyRowExpSolvableEnhancedFinder, (inVariables,false,isInitial,mark,rowmark,inRow));
...
protected function traversingAdjacencyRowExpSolvableEnhancedFinder "Helper for adjacencyRowExpEnhanced"
  input DAE.Exp inExp;
  input tuple<BackendDAE.Variables, Boolean, Boolean, Integer, array<Integer>, list<Integer>> inTpl "(exp, (variables, unsolvable, mark, rowmark, row))";
  output DAE.Exp outExp;
  output Boolean cont;
  output tuple<BackendDAE.Variables, Boolean, Boolean, Integer, array<Integer>, list<Integer>> outTpl;
algorithm
  (outExp, cont, outTpl) := matchcontinue (inExp, inTpl)
    local
    ...
    case (DAE.LUNARY(exp=e1), (vars, bs, isInitial, it, at, pa)) algorithm
    ...
    case (DAE.LBINARY(exp1=e1, exp2=e2), (vars, bs, isInitial, it, at, pa)) algorithm
    ...
    case (DAE.RELATION(exp1=e1, exp2=e2), (vars, bs, isInitial, it, at, pa)) algorithm
      (_, (vars, _, _, _, _, pa)) := Expression.traverseExpTopDown(e1, traversingAdjacencyRowExpSolvableEnhancedFinder, (vars, true, isInitial, it, at, pa));
      (_, (vars, _, _, _, _, pa)) := Expression.traverseExpTopDown(e2, traversingAdjacencyRowExpSolvableEnhancedFinder, (vars, true, isInitial, it, at, pa));
    then (inExp, false, (vars, bs, isInitial, it, at, pa));
    ...
    case (DAE.RANGE(start=e1, step=SOME(e3), stop=e2), (vars, bs, isInitial, it, at, pa)) algorithm
      (_, (vars, _, _, _, _, pa)) := Expression.traverseExpTopDown(e1, traversingAdjacencyRowExpSolvableEnhancedFinder, (vars, true, isInitial, it, at, pa));
      (_, (vars, _, _, _, _, pa)) := Expression.traverseExpTopDown(e2, traversingAdjacencyRowExpSolvableEnhancedFinder, (vars, true, isInitial, it, at, pa));
    ...
    case (DAE.ASUB(exp=e1, sub={DAE.INDEX(DAE.ICONST(i))}), (vars, bs, isInitial, it, at, pa)) algorithm
      e1 := Expression.nthArrayExp(e1, i);
      (_, (vars, _, _, _, _, pa)) := Expression.traverseExpTopDown(e1, traversingAdjacencyRowExpSolvableEnhancedFinder, (vars, bs, isInitial, it, at, pa));
    then (inExp, false, (vars, bs, isInitial, it, at, pa));

    case (DAE.ASUB(), (_, _, _, _, _, _))
    then fail();

    case (DAE.CREF(componentRef=cr), (vars, bs, isInitial, it, at, pa)) algorithm
      mark := it;
      rowmark := at;
      (varslst, p) := BackendVariable.getVar(cr, vars);
      res := adjacencyRowExpEnhanced1(varslst, p, pa, true, mark, rowmark, bs);
    then (inExp, false, (vars, bs, isInitial, it, at, res));

    case (DAE.CALL(path=Absyn.IDENT(name="der"), expLst={DAE.CREF(componentRef=cr)}), (vars, bs, isInitial, it, at, pa)) algorithm
      mark := it;
      rowmark := at;
      (varslst, p) := BackendVariable.getVar(cr, vars);
    ...
    case (DAE.CALL(path=Absyn.IDENT(name="der"), expLst={DAE.CREF(componentRef=cr), DAE.ICONST(_)}), (vars, bs, isInitial, it, at, pa)) algorithm
      mark := it;
      rowmark := at;
      (varslst, p) := BackendVariable.getVar(cr, vars);
      res := adjacencyRowExpEnhanced1(varslst, p, pa, false, mark, rowmark, bs);
...
algorithm
  (ovar, outTypeA) := matchcontinue(inVar)
    ...
    case SOME(BackendDAE.VAR(cref, varKind, varDirection, varParallelism, varType, SOME(e1), tplExp, instdims, source, attr, ts, hideResult, comment, ct, io, unreplaceable, initNonlinear, encrypted)) algorithm
    ...
    case SOME(BackendDAE.VAR(cref, varKind, varDirection, varParallelism, varType, NONE(), tplExp, instdims, source, attr, ts, hideResult, comment, ct, io, unreplaceable, initNonlinear, encrypted)) algorithm
...
algorithm
 (outAttr,outExtraArg) := match(attr,func,extraArg)
   ...
   case(SOME(DAE.VAR_ATTR_REAL(q,u,du,min,max,i,f,n,ss,unc,dist,eqbound,p,fin,startOrigin)),_,_) algorithm
   ...
   case(SOME(DAE.VAR_ATTR_INT(q,min,max,i,f,unc,dist,eqbound,p,fin,startOrigin)),_,_) algorithm
     (q_,outExtraArg) := Expression.traverseExpOpt(q,func,extraArg);
     (min_,outExtraArg) := Expression.traverseExpOpt(min,func,outExtraArg);
     (max_,outExtraArg) := Expression.traverseExpOpt(max,func,outExtraArg);
     (i_,outExtraArg) := Expression.traverseExpOpt(i,func,outExtraArg);
   ...
   case(SOME(DAE.VAR_ATTR_BOOL(q,i,f,eqbound,p,fin,startOrigin)),_,_) algorithm
     (q_,outExtraArg) := Expression.traverseExpOpt(q,func,extraArg);
     (i_,outExtraArg) := Expression.traverseExpOpt(i,func,outExtraArg);
     (f_,outExtraArg) := Expression.traverseExpOpt(f,func,outExtraArg);
     (eqbound_,outExtraArg) := Expression.traverseExpOpt(eqbound,func,outExtraArg);
   ...
   case(SOME(DAE.VAR_ATTR_STRING(q,i,f,eqbound,p,fin,startOrigin)),_,_) algorithm
     (q_,outExtraArg) := Expression.traverseExpOpt(q,func,extraArg);
     (i_,outExtraArg) := Expression.traverseExpOpt(i,func,outExtraArg);
     (f_,outExtraArg) := Expression.traverseExpOpt(f,func,outExtraArg);
     (eqbound_,outExtraArg) := Expression.traverseExpOpt(eqbound,func,outExtraArg);
   ...
   case(SOME(DAE.VAR_ATTR_ENUMERATION(q,min,max,i,f,eqbound,p,fin,startOrigin)),_,_) algorithm
     (q_,outExtraArg) := Expression.traverseExpOpt(q,func,extraArg);
     (min_,outExtraArg) := Expression.traverseExpOpt(min,func,outExtraArg);
     (max_,outExtraArg) := Expression.traverseExpOpt(max,func,outExtraArg);
     (i_,outExtraArg) := Expression.traverseExpOpt(i,func,outExtraArg);
...
algorithm
 ...
 local
   ...
   case(SOME(DAE.DISTRIBUTION(name,arr,sarr)),_,_) algorithm
...
algorithm
  (osysts,oshared,oargs,oCausalized) := match (isysts,ishared,inMatchingOptions,matchingAlgorithm,stateDeselection,acc,acc1,iCausalized)
    ...
    case (syst::systs,_,_,_,_,_,_,_) algorithm
...
algorithm
  (osyst,oshared,oArg,oCausalized) := matchcontinue (isyst,matchingAlgorithm,stateDeselection)
    ...
    case (BackendDAE.EQSYSTEM(matching=BackendDAE.NO_MATCHING()), (matchingAlgorithmfunc,_), (sssHandler,_,_,_)) algorithm
      //BackendDump.dumpEqSystem(isyst, "causalizeDAEWork");
      //print("SystemSize: " + intString(systemSize(isyst)));
      funcs := getFunctions(ishared);
      (syst,_,_,mapEqnIncRow,mapIncRowEqn) := getAdjacencyMatrixScalar(isyst,BackendDAE.SOLVABLE(), SOME(funcs), isInitializationDAE(ishared));
...
algorithm
  ...
  (m, _) := BackendDAEUtil.adjacencyMatrix(paramSystem, BackendDAE.NORMAL(), NONE(), isInitializationDAE(backendDAE.shared));
...
algorithm
  (outConditionVarList, outInitialCall) := match inConditionList
    ...
    case exp::conditionList guard(Expression.isConst(exp)) algorithm
      (conditionVarList, initialCall) := getConditionList1(conditionList, inConditionVarList, inInitialCall);
    then (conditionVarList, initialCall);

    case DAE.CALL(path=Absyn.IDENT(name="initial"))::conditionList algorithm
      (conditionVarList, initialCall) := getConditionList1(conditionList, inConditionVarList, true);
    then (conditionVarList, initialCall);

    case DAE.CREF(componentRef=componentRef)::conditionList algorithm
...
algorithm
  i := match(inExp)
    ...
    case DAE.BINARY(exp1=e1, operator=DAE.ADD(DAE.T_INTEGER()), exp2=e2) algorithm
      i1 := expInt(e1, inKnVariables);
      i2 := expInt(e2, inKnVariables);
      i := i1 + i2;
    then i;

    case DAE.BINARY(exp1=e1, operator=DAE.SUB(DAE.T_INTEGER()), exp2=e2) algorithm
      i1 := expInt(e1, inKnVariables);
      i2 := expInt(e2, inKnVariables);
      i := i1 - i2;
    then i;
...
algorithm
  ...
  (m, mT) := BackendDAEUtil.adjacencyMatrixDispatch(BackendVariable.listVar1(varLstIn), BackendEquation.listEquation(eqs), BackendDAE.ABSOLUTE(), NONE(), isInitial);
...
protected
  ...
  BackendDAE.AdjacencyMatrix m, mT, mOrig, mTOrig;
...
algorithm
  ...
  (_, mOrig, mTOrig) := BackendDAEUtil.getAdjacencyMatrixfromOption(syst, BackendDAE.ABSOLUTE(), SOME(functionTree), isInitial);
  ...
  fail();
end checkAdjacencyMatrixSolvability;
